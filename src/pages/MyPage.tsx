...
const MyPage: React.FC = () => {
  ...
  const handleDownloadAndDispose = async () => {
    ...
    try {
      ...
      link.click();
      const { error } = await supabase.from('coupons').update({ is_used: true, used_at: new Date().toISOString(), serial_number: generatedSerial }).eq('id', selectedCoupon.id);
      if (error) throw error;
      alert('QR 저장이 완료되었습니다. 해당 쿠폰은 자동 소멸됩니다.');
  ...
  if (authLoading || !currentUser) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-black italic text-red-600 animate-pulse uppercase tracking-widest">데이터 동기화 중...</div>;
  }
    ...
    <div className="min-h-screen bg-[#050505] pt-32 pb-20 px-6 font-sans text-white selection:bg-red-600/30 relative">
      ...
      <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700">
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
            <div className="relative group w-40 h-40 shrink-0">
              <div className="w-full h-full rounded-[3rem] bg-gradient-to-br from-[#1a1a1a] to-black border-2 border-white/10 flex items-center justify-center overflow-hidden shadow-2xl">
                {currentUser.avatar_url ? <img src={currentUser.avatar_url} className="w-full h-full object-cover" /> : <span className="text-6xl text-red-600 italic font-black">{currentUser.nickname?.[0].toUpperCase()}</span>}
              </div>
            ...
            <div className="flex-grow text-center md:text-left">
              <div className="inline-block bg-red-600 text-[10px] font-black px-4 py-1.5 rounded-full uppercase italic mb-4 shadow-lg">LV.{currentUser.level} {LEVEL_NAMES[currentUser.level]}</div>
              <div className="flex items-center justify-center md:justify-start gap-4 mb-4">
                {isEditing ? (
                  ...
                  <><h2 className="text-5xl font-black italic tracking-tighter uppercase leading-none">{currentUser.nickname}님</h2><button onClick={() => setIsEditing(true)} className="text-gray-600 hover:text-white transition-colors text-xl p-2 bg-white/5 rounded-xl">✏️</button></>
                )}
              </div>
            ...
            {currentCriteria && (
              <div className="w-full md:w-64 bg-black/40 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
                <p className="text-[10px] font-black text-yellow-500 uppercase mb-6 tracking-widest italic border-b border-white/5 pb-2">NEXT GOAL: {LEVEL_NAMES[currentUser.level + 1]}</p>
                <div className="space-y-6">
                  <div><div className="flex justify-between text-[9px] mb-2 font-black uppercase italic tracking-tighter"><span>포인트 달성도</span><span className="text-red-500">{currentUser.points}/{currentCriteria.points}</span></div><div className="h-1.5 bg-black rounded-full overflow-hidden border border-white/5 shadow-inner"><div className="bg-red-600 h-full transition-all duration-1000" style={{ width: `${Math.min(100, (currentUser.points / currentCriteria.points) * 100)}%` }}></div></div></div>
                  <div><div className="flex justify-between text-[9px] mb-2 font-black uppercase italic tracking-tighter"><span>후기 달성도</span><span className="text-emerald-500">{currentUser.review_count || 0}/{currentCriteria.reviews}</span></div><div className="h-1.5 bg-black rounded-full overflow-hidden border border-white/5 shadow-inner"><div className="bg-emerald-600 h-full transition-all duration-1000" style={{ width: `${Math.min(100, ((currentUser.review_count || 0) / currentCriteria.reviews) * 100)}%` }}></div></div></div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="bg-[#0f0f0f] rounded-[3rem] border border-white/5 overflow-hidden shadow-2xl">
          ...
          <div className="p-6 md:p-10 min-h-[400px]">
            {dataLoading ? <div className="text-center py-20 animate-pulse">정보 불러오는 중...</div> : (
              <div className="animate-in fade-in duration-500">
                {activeTab === 'activity' && (
                  <div className="space-y-4">
                    {myPosts.length > 0 ? myPosts.map(post => (
                      <Link key={post.id} to={`/post/${post.id}`} className="flex justify-between items-center p-6 bg-white/[0.03] rounded-3xl border border-white/5 hover:border-red-600/40 transition-all group shadow-lg">
                        <div className="flex items-center gap-4"><span className="text-red-600 font-black text-[9px] px-2 py-1 bg-red-600/10 rounded-lg">#{post.category}</span><span className="text-sm font-bold truncate group-hover:text-red-500 italic tracking-tight">{post.title}</span></div>
                        <span className="text-[9px] text-gray-600 font-black uppercase italic shrink-0 ml-4">{new Date(post.created_at).toLocaleDateString()}</span>
                      </Link>
                    )) : <div className="py-32 text-center opacity-40 italic font-black uppercase tracking-widest">기록된 게시글이 없습니다.</div>}
                  </div>
                )}
                {activeTab === 'points' && (
                  <div className="space-y-4">
                    {pointHistory.length > 0 ? pointHistory.map(item => (
                      <div key={item.id} className="p-6 bg-white/[0.03] rounded-3xl flex justify-between items-center border border-white/5">
                        <div className="space-y-1"><span className="font-black block italic text-sm uppercase tracking-tighter">{item.reason}</span><span className="text-[9px] text-gray-600 font-bold uppercase italic">{new Date(item.created_at).toLocaleString()}</span></div>
                        <span className={`text-xl font-black italic ${item.amount > 0 ? 'text-emerald-500' : 'text-red-600'}`}>{item.amount > 0 ? '+' : ''}{item.amount.toLocaleString()}P</span>
                      </div>
                    )) : <div className="py-32 text-center opacity-40 italic font-black uppercase tracking-widest">포인트 내역이 없습니다.</div>}
                  </div>
                )}
                {activeTab === 'coupons' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {myCoupons.length > 0 ? myCoupons.map(coupon => (
                      <div key={coupon.id} className="p-8 bg-gradient-to-br from-[#1a1a1a] to-black rounded-[3rem] border border-red-600/20 group hover:border-red-600/50 transition-all shadow-2xl relative overflow-hidden">
                        ...
                        <h4 className="text-2xl font-black text-white italic mb-4 tracking-tighter uppercase leading-none">{coupon.title}</h4>
                        <p className="text-gray-500 text-xs font-bold leading-relaxed mb-10 italic">{coupon.content}</p>
                        <div className="pt-6 border-t border-white/5 flex justify-between items-center"><span className="text-[9px] text-gray-600 font-black italic uppercase tracking-[0.2em]">만료일: {new Date(coupon.expired_at).toLocaleDateString()}</span><button onClick={() => openCouponModal(coupon)} className="bg-red-600 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase italic shadow-xl hover:bg-red-500 transition-all">사용하기</button></div>
                      </div>
        ...
        {/* 🔴 하단 액션 버튼 수정: 모바일에서 세로 배치로 UI 깨짐 해결 */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 px-4 md:px-10">
          <Link to="/" className="text-gray-600 hover:text-white text-[10px] font-black uppercase italic tracking-[0.3em] transition-all border-b border-transparent hover:border-white whitespace-nowrap">
            ← 메인으로 돌아가기
          </Link>
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            {currentUser.role === UserRole.ADMIN && (
              <button onClick={() => navigate('/admin')} className="flex-1 px-10 py-5 bg-white/5 rounded-2xl text-[10px] font-black uppercase border border-white/10 italic hover:bg-white/10 transition-all tracking-[0.2em] shadow-xl">
                관리자 메뉴
              </button>
            )}
            <button onClick={handleLogout} className="flex-1 px-10 py-5 bg-red-600 rounded-2xl text-[10px] font-black uppercase italic shadow-2xl shadow-red-900/40 hover:bg-red-700 active:scale-95 transition-all tracking-[0.2em]">
              로그아웃
            </button>
          </div>
        </div>
      ...
      {selectedCoupon && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md animate-in fade-in duration-300">
          <div className="relative bg-[#0a0a0a] border border-red-600/30 p-10 rounded-[3.5rem] max-w-[400px] w-full text-center shadow-[0_0_50px_rgba(220,38,38,0.2)]">
