...
const MyPage: React.FC = () => {
  ...
  const handleDownloadAndDispose = async () => {
    ...
    try {
      ...
      await fetchMyData(); 
    } catch (err) { alert('처리 중 오류 발생'); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] pt-32 pb-20 px-6 font-sans text-white selection:bg-red-600/30 relative">
      <Helmet>
        ...
        <meta name="description" content="호놀자 회원님의 활동 내역과 쿠폰을 관리하세요." />
      </Helmet>
            ...
            {currentCriteria && (
              <div className="w-full md:w-64 bg-black/40 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
                ...
                <div className="space-y-6">
                  <div>
                    <div className="flex justify-between text-[9px] mb-2 font-black uppercase italic tracking-tighter"><span>포인트 달성도</span><span className="text-red-500">{currentUser.points}/{currentCriteria.points}</span></div>
                  ...
                  <div>
                    <div className="flex justify-between text-[9px] mb-2 font-black uppercase italic tracking-tighter"><span>후기 달성도</span><span className="text-emerald-500">{currentUser.review_count || 0}/{currentCriteria.reviews}</span></div>
                    <div className="h-1.5 bg-black rounded-full overflow-hidden border border-white/5 shadow-inner">
          ...
          <div className="p-10 min-h-[450px]">
            {dataLoading ? (
              <div className="flex flex-col items-center justify-center h-[300px] text-gray-800 font-black italic animate-pulse space-y-4">
                ...
                <span className="tracking-[0.3em] text-[10px]">정보를 불러오는 중...</span>
              </div>
            ) : (
              <div className="animate-in fade-in duration-500">
                {activeTab === 'activity' && (
                  <div className="space-y-4">
                    {myPosts.length > 0 ? myPosts.map(post => (
                      <Link key={post.id} to={`/post/${post.id}`} className="flex justify-between items-center p-8 bg-white/[0.03] rounded-[2.5rem] border border-white/5 hover:border-red-600/40 transition-all group shadow-lg">
                        <div className="flex items-center gap-6">
                          ...
                          <span className="text-xl font-bold group-hover:text-red-500 transition-colors italic tracking-tight leading-tight">{post.title}</span>
                        </div>
                    ...
                    )) : <div className="py-32 text-center bg-black/20 rounded-[2.5rem] border border-dashed border-white/5 opacity-40"><p className="text-gray-700 font-black italic uppercase tracking-widest text-lg">기록된 게시글이 없습니다.</p></div>}
                  </div>
                )}
                ...
                {activeTab === 'points' && (
                  <div className="space-y-4">
                    ...
                    )) : <div className="py-32 text-center bg-black/20 rounded-[2.5rem] border border-dashed border-white/5 opacity-40"><p className="text-gray-700 font-black italic uppercase tracking-widest text-lg">포인트 내역이 없습니다.</p></div>}
                  </div>
                )}
                ...
                {activeTab === 'coupons' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    )) : <div className="col-span-2 py-32 text-center bg-black/20 rounded-[3rem] border border-dashed border-white/10 opacity-40"><p className="text-gray-700 font-black italic uppercase tracking-widest text-lg">보유하신 쿠폰이 없습니다.</p></div>}
                  </div>
                )}
              </div>
            )}
