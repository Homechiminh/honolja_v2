...
const Home: React.FC = () => {
  ...
  // 2. 하단 Premium Stays용 숙소 필터링 (4개 노출로 확장하여 4열 배치)
  const villaStores = stores.filter(s => s.category === 'villa').slice(0, 4);

  /**
   * 🔴 [방탄 fetch] 게시글/공지사항 최신순 6개 로드
   */
  const fetchHomeData = async () => {
    try {
      // 병렬 호출로 최신 데이터 각 6개씩 수집
    ...
    <div className="w-full bg-[#050505] relative overflow-hidden selection:bg-red-600/30">
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-2 flex lg:flex-col gap-4">
          <a href={SNS_LINKS.telegram} target="_blank" rel="noreferrer" className="flex-1 bg-[#0088cc] rounded-[2rem] p-8 relative overflow-hidden group hover:scale-[1.03] transition-all shadow-xl">
          ...
          <a href={SNS_LINKS.kakao} target="_blank" rel="noreferrer" className="flex-1 bg-[#FEE500] rounded-[2rem] p-8 relative overflow-hidden group hover:scale-[1.03] transition-all text-black shadow-xl">
      ...
      {/* [섹션 4] PREMIUM STAYS - 절반 크기(4열 배치) 적용 */}
      <section className="max-w-[1400px] mx-auto px-6 py-24 mb-20">
        <div className="bg-[#080808] rounded-[3rem] p-12 border border-white/5 relative overflow-hidden shadow-2xl">
          ...
          {/* 🔴 grid-cols-2 md:grid-cols-4로 변경하여 카드 크기를 절반으로 축소 */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 relative z-10 px-4">
            {storesLoading ? (
              [1, 2, 3, 4].map(i => <div key={i} className="aspect-video bg-white/5 rounded-[1.5rem] animate-pulse" />)
              ...
              <div className="col-span-4 py-20 text-center border-2 border-dashed border-white/5 rounded-[2.5rem]">
