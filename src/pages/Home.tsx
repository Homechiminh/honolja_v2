...
const Home: React.FC = () => {
  ...
  // 🔴 데이터 가져오기 함수 (인증과 별개로 즉시 실행 가능하도록 분리)
  const fetchHomeData = async () => {
    try {
      ...
      if (postRes.data) {
        console.log(`✅ 일반글 수신: ${postRes.data.length}개`);
        setLatestPosts(postRes.data);
      }
      if (vipRes.data) setLatestVipPosts(vipRes.data);
      if (noticeRes.data) {
        console.log(`✅ 공지사항 수신: ${noticeRes.data.length}개`);
  ...
  // 🔴 최적화 포인트: 컴포넌트 마운트 시 즉시 데이터 호출 (인증 안 기다림)
  useEffect(() => {
    fetchHomeData();
  }, []); 
  ...
  // 🔴 초기화 중일 때 아주 가벼운 로딩만 노출 (2초 이상 걸리지 않게 방어)
  if (!initialized) return (
    <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center">
      <div className="w-10 h-10 border-4 border-red-600 border-t-transparent rounded-full animate-spin mb-4"></div>
      <div className="text-red-600 font-black animate-pulse italic uppercase text-[10px] tracking-widest">
        Verifying Security...
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full bg-[#050505] relative overflow-hidden selection:bg-red-600/30 font-sans text-white">
      <Helmet>
        ...
        <meta name="description" content="호치민 밤문화, 유흥, 풀빌라 예약까지 한 번에 해결하세요." />
      </Helmet>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
          {storesLoading ? [1,2,3,4,5].map(i => <div key={i} className="aspect-[3/4] bg-white/5 rounded-[24px] animate-pulse" />) : 
           ...
           <div className="col-span-full py-10 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
             <p className="text-gray-500 italic">표시할 데이터가 없습니다. (Console을 확인하세요)</p>
           </div>}
        </div>
      </section>
        ...
        <div className="lg:col-span-10 grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <div className="bg-[#111] rounded-2xl border border-white/5 divide-y divide-white/5 overflow-hidden shadow-2xl">
              ...
              )) : <div className="p-4 text-xs text-gray-600 italic">불러오는 중...</div>}
            </div>
          </div>
