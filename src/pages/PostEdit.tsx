...
const PostEdit: React.FC = () => {
  ...
  // 🔴 내용 변경 시 세션 스토리지에 실시간 저장 (탭 전환 대비)
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#050505] pt-32 pb-20 px-6 font-sans selection:bg-red-600/30">
      <div className="max-w-4xl mx-auto bg-[#0f0f0f] rounded-[3rem] p-10 md:p-16 border border-white/5 shadow-2xl">
        <div className="flex justify-between items-center mb-10">
          ...
          <span className="text-[10px] text-emerald-500 font-bold animate-pulse">● 실시간 임시저장 중</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-700">
          ...
          <div className="flex gap-4 pt-4">
            <button 
              type="button" 
              onClick={() => { sessionStorage.removeItem(`post_edit_draft_${id}`); navigate(-1); }} 
            ...
            <button 
              type="submit" 
              disabled={updating} 
              className="flex-[2] py-6 bg-red-600 text-white font-black text-xl rounded-2xl uppercase shadow-2xl hover:bg-red-500 transition-all active:scale-95 italic"
            >
              {updating ? '수정 중...' : '게시글 수정'}
            </button>
          </div>
        </form>
      </div>
