...
const Login: React.FC = () => {
  ...
  // 🔴 initialized를 가져와서 더 정확하게 상태를 파악합니다.
  ...
  // 이미 로그인된 유저 리다이렉트 (기존 로직 유지하되 initialized 기준)
  useEffect(() => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🔴 [핵심 수정] 200 성공 신호를 받으면 리액트 상태 변화를 기다리지 않고
      // 즉시 페이지를 홈으로 강제 전환합니다. (무한 뺑뺑이 방지)
      if (data.user) {
        console.log("✅ 서버 승인 완료! 홈으로 이동합니다.");
        // 가끔 navigate가 씹히는 경우를 대비해 확실한 명령을 내립니다.
      ...
      // 에러가 났을 때만 로딩을 풀어줍니다. (성공 시에는 리다이렉트되므로 필요 없음)
      setIsLoading(false);
    }
  };

  // 초기화 중에는 화면을 가려서 폼이 깜빡이는 걸 막습니다.
      ...
      <div className="max-w-md w-full relative z-10 animate-in fade-in duration-700">
        ...
        <div className="bg-[#111] p-8 md:p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
          <div className="mb-8">
            ...
            >
              {/* 🔴 SVG 패스 오타(7.07|3.66)를 완벽히 교정한 버전입니다. */}
              <svg className="w-6 h-6" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07 9.5 12l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
