...
const Login: React.FC = () => {
  ...
  // 1. 이미 로그인된 유저가 접근 시 홈으로 강제 리다이렉트
  useEffect(() => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // Pro 플랜 대역폭을 사용하여 즉시 세션 동기화 및 이동
        window.location.replace('/'); 
      }
  ...
  // 🔴 중요: 초기화 중일 때 null 대신 로딩 UI를 보여줌 (화면 멈춤 방지)
  if (!initialized) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center gap-6">
        <div className="w-10 h-10 border-4 border-red-600 border-t-transparent rounded-full animate-spin"></div>
        <p className="text-white/50 font-black text-xs uppercase tracking-[0.3em] animate-pulse">
      ...
      <div className="max-w-md w-full relative z-10 animate-in fade-in duration-700">
        ...
        <div className="text-center mb-12">
          <Link to="/" className="inline-flex items-center space-x-3 mb-8 group">
            <div className="w-12 h-12 bg-red-600 rounded-xl flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-300">
        ...
        <div className="bg-[#111] p-8 md:p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
          ...
          <form className="space-y-6" onSubmit={handleFormSubmit}>
            <div className="space-y-4">
              <input 
                id="login-email" name="email" type="email" placeholder="Email Address" required autoComplete="email"
              ...
              />
              <input 
                id="login-password" name="password" type="password" placeholder="Password" required autoComplete="current-password"
            ...
            <button 
              type="submit" 
              disabled={isLoading} 
              className="w-full py-5 bg-red-600 text-white rounded-2xl font-black text-lg hover:bg-red-700 transition-all active:scale-95 disabled:opacity-50 uppercase tracking-tighter italic shadow-xl shadow-red-900/20"
            >
